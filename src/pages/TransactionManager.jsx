...
// 🎵 CONFIGURAZIONE (in produzione sarebbe importata)
const CONFIG = {
  brand: {
  ...
  services: {
    "rec-voice": { name: "Registrazione Voce", pricePerHour: 50, category: "Registrazione", icon: "🎤" },
    "rec-instruments": { name: "Registrazione Strumenti", pricePerHour: 60, category: "Registrazione", icon: "🎸" },
    "mix-master": { name: "Mix & Master", pricePerHour: 80, category: "Post-Produzione", icon: "🎛️" },
    "beat-prod": { name: "Produzione Beat", pricePerHour: 70, category: "Produzione", icon: "🎹" },
    "editing": { name: "Editing Audio", pricePerHour: 45, category: "Post-Produzione", icon: "✂️" },
    "vocal-tuning": { name: "Vocal Tuning", pricePerHour: 40, category: "Post-Produzione", icon: "🎵" }
  },
  paymentMethods: [
    { id: "cash", name: "Contanti", icon: "💵" },
    { id: "bank", name: "Bonifico", icon: "🏦" },
    { id: "paypal", name: "PayPal", icon: "💙" },
    { id: "card", name: "Carta", icon: "💳" },
    { id: "satispay", name: "Satispay", icon: "📱" }
  ],
  statuses: {
    pending: { name: "In Attesa", icon: "⏳", bg: "#FEF3C7", color: "#92400E" },
    partial: { name: "Parziale", icon: "💰", bg: "#DBEAFE", color: "#1E40AF" },
    paid: { name: "Pagato", icon: "✓", bg: "#D1FAE5", color: "#065F46" }
  },
  storageKeys: {
...
const TransactionManager = () => {
  ...
  const calculateTotal = (services) => {
    return services.reduce((sum, s) => {
      const service = CONFIG.services[s.serviceId];
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
            <div>
              <h1 style={{ margin: '0 0 5px 0', fontSize: '2rem', fontWeight: 700 }}>
                {CONFIG.brand.logo} Transaction Manager
              </h1>
                  ...
                  >
                    ...
                    <div style={{ 
                      ...
                      {transaction.services.map((service, idx) => (
                        ...
                        >
                          {CONFIG.services[service.serviceId]?.icon} {service.name} ({service.hours}h)
                        </span>
                      ))}
                    </div>
            ...
            <div style={{ display: 'grid', gap: '20px' }}>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                  ...
                  <button
                    onClick={() => setTransactionServices(prev => [...prev, { serviceId: Object.keys(CONFIG.services)[0], hours: 1 }])}
                    style={{
                      padding: '8px 16px',
                  ...
                  >
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr auto', gap: '10px', alignItems: 'end' }}>
                      <div>
                        ...
                        >
                          {Object.entries(CONFIG.services).map(([id, s]) => (
                    ...
                    <div style={{ marginTop: '8px', fontSize: '0.9rem', color: '#666', textAlign: 'right' }}>
                      Subtotale: <strong style={{ color: CONFIG.brand.colors.primary }}>
                        €{(CONFIG.services[service.serviceId].pricePerHour * service.hours).toFixed(2)}
                      </strong>
                    </div>
                  </div>
                ))}
              ...
              {selectedTransaction.services.map((service, idx) => (
                ...
                >
                  <span style={{ color: '#666' }}>
                    {CONFIG.services[service.serviceId]?.icon} {service.name} ({service.hours}h @ €{service.price}/h)
                  </span>
