...
const ClientHub = () => {
  ...
  // 💾 SAVE DATA
  useEffect(() => {
    localStorage.setItem(CONFIG.storageKeys.artists, JSON.stringify(artists));
  }, [artists]);
  ...
  const addArtist = () => {
    if (!formData.name || !formData.artistName) {
      alert('Nome e Nome d\'Arte sono obbligatori');
      return;
    }
  ...
  const updateArtist = () => {
    ...
    setArtists(prev => prev.map(a => 
      a.id === selectedArtist.id ? { ...a, ...formData } : a
    ));
    
    setSelectedArtist(prev => ({ ...prev, ...formData }));
    setView('detail');
  };
  ...
  const deleteArtist = (artistId) => {
    if (window.confirm('Sei sicuro di voler eliminare questo artista? Verranno eliminate anche tutte le sue transazioni.')) {
      ...
      setTransactions(prev => prev.filter(t => t.artistId !== artistId));
  ...
  const addNote = () => {
    ...
    setSelectedArtist(prev => ({
      ...prev,
      noteHistory: [note, ...(prev.noteHistory || [])]
    }));
